...
public class InAppWebViewChromeClient extends WebChromeClient implements PluginRegistry.ActivityResultListener, ActivityResultListener {
  ...
  private boolean isFileNotEmpty(Uri uri, @Nullable File file) {
    ...
    try {
      ...
      if (descriptor != null) {
        ...
        if (length == AssetFileDescriptor.UNKNOWN_LENGTH) {
          ParcelFileDescriptor pfd = descriptor.getParcelFileDescriptor();
          if (pfd != null) {
            length = pfd.getStatSize();
          }
        }
        descriptor.close();
        if (length > 0) {
      ...
      Log.d(LOG_TAG, "Cannot check file length via descriptor: " + e.getMessage());
    }

    // Fallback: check if we can open an InputStream and read at least 1 byte
    try {
      ...
      Log.d(LOG_TAG, "Cannot open InputStream: " + e.getMessage());
    }
  ...
  private Uri getCapturedMediaFile() {
    if (imageOutputFileUri != null && isFileNotEmpty(imageOutputFileUri, imageOutputFile)) {
      return imageOutputFileUri;
    }

    if (videoOutputFileUri != null && isFileNotEmpty(videoOutputFileUri, videoOutputFile)) {
  ...
  private Intent getPhotoIntent() {
    ...
    if (imageOutputFileUri != null) {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
        intent.setClipData(ClipData.newRawUri("", imageOutputFileUri));
      }
      Activity activity = getActivity();
      if (activity != null) {
        ...
        for (ResolveInfo resolveInfo : resInfoList) {
          ...
          activity.grantUriPermission(packageName, imageOutputFileUri, Intent.FLAG_GRANT_WRITE_URI_PERMISSION | Intent.FLAG_GRANT_READ_URI_PERMISSION);
        }
      }
    }
    return intent;
  ...
  private Intent getVideoIntent() {
    ...
    if (videoOutputFileUri != null) {
      ...
      intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
        intent.setClipData(ClipData.newRawUri("", videoOutputFileUri));
      }
      Activity activity = getActivity();
      if (activity != null) {
        List<ResolveInfo> resInfoList = activity.getPackageManager().queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
        for (ResolveInfo resolveInfo : resInfoList) {
          String packageName = resolveInfo.activityInfo.packageName;
          activity.grantUriPermission(packageName, videoOutputFileUri, Intent.FLAG_GRANT_WRITE_URI_PERMISSION | Intent.FLAG_GRANT_READ_URI_PERMISSION);
        }
      }
    }
    return intent;
